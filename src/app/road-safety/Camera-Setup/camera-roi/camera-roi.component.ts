...
export class CameraRoiComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // parkingROIColor: string = "#4eb078"
  parkingROIColor: string =" #38be72"
  ...
  polygonOptions: any = {
   ...
   classIds: any[] = [];
  // classIds: any[] 
  classIDPerson: FormControl = new FormControl(0, Validators.required);
  ...
  AddNewROI() {
    ...
    this.isAddCC = false;
    console.log(this.setTimeOption,'this is from the add new roi and this is the object of the settimeoption---------')
  }
  ...
  ClassId(event: any) {
    console.log('this is event from the classid -----',event.target.defaultValue)
   
    if (event.target.checked) {
      this.classIds.push(event.target.value);
      console.log('this is this from the if ??????????????????',this.classIds)
    } else {
      var index = this.classIds.indexOf(event.target.defaultValue);
      this.classIds.splice(index, 1);
      console.log('this is this from the else----------------',this.classIds)
    }
  }
  ...
  datesUpdated(dateEvent: any) {
    this.selectedMoments = dateEvent
