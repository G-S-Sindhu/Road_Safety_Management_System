...
export interface ROI {
  ...
  class_id?: any[];
  roi_canvas?: any;
  alarm_type?:{hooter:any,relay:any}
}
@Component({
...
export class CameraRoiComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  deleteField: any = "";
  selectedCCMode: any;
  isAddRelay: FormControl = new FormControl(null, Validators.required);
  isAddHooter: FormControl = new FormControl(null, Validators.required);
  CCObjectCounts: any[] = [];
  CCObjectsCount: any[] = [];
      ...
      items: [
        {
          label: "ROI",
          command: (onclick: any) => {
            console.log(onclick), this.AddCCRoi();
          },
        },
        {
          label: "Full Frame",
        ...
        },
        {
          label: "Full Frame",
          command: (onclick: any) => {
            console.log(onclick), this.AddCCForFrame();
          },
        },
      ],
    },
  ...
  ngAfterViewInit(): void {
    ...
    this.ccObjectsMinMax[0].valueChanges.subscribe(() => {
        ...
        if (
          this.ccObjectsMinMax[0].get("min").value > this.ccObjectsMinMax[0].get("max").value ||
          this.ccObjectsMinMax[0].get("min").value == this.ccObjectsMinMax[0].get("max").value
        ) {
          this.CCValid = 0;
        }
  ...
  ModalOpen(modal: any) {
    ...
    if (this.cameraData.length > 0) {
      console.log(this.cameraData[0].alarm_ip_address);
  ...
  getClickCoords(event: any) {
    console.log(event, "clicking coordinates");
        ...
        if (ROIName !== null) {
          ...
          var tempObj: any = {
            ...
            roi_data: {
              ...
              alarm_type: {
                hooter: this.isAddHooter.value,
                relay: this.isAddRelay.value,
              },
              alarm_ip_address: {
                hooter_ip: this.cameraData[0].alarm_ip_address.hooter_ip,
                relay_ip: this.cameraData[0].alarm_ip_address.relay_ip,
              },
            },
          };
        }
  ...
  DeleteROI() {
    if (this.deleteField == "RA") {
      ...
      console.log(this.deleteID, this.spillageRois[this.deleteID]);
      ...
      this.server.DeleteSpillageRoi(data).subscribe(
        (response: any) => {
          if (response.success) {
            ...
            this.server.notification(response.message);
            console.log(
              this.spillageRois[this.deleteID].roi_canvas,
  ...
  SetRelayOrHooter(event: any,index:any, data?: any) {
    this.selectedRoi = data;
    this.selectedEditId=index
    if (event.target.checked) {
      typeof (this.allCameraData[this.selectedEditId].alarm_type.hooter)=='boolean'?this.allCameraData[this.selectedEditId].alarm_type.hooter=true:this.selectedRoi.alarm_type.relay=='boolean'?this.selectedRoi.alarm_type.relay=true:'';
    }
    else{
      typeof this.allCameraData[this.selectedEditId].alarm_type.hooter=='boolean'?this.allCameraData[this.selectedEditId].alarm_type.hooter=false:this.selectedRoi.alarm_type.relay=='boolean'?this.selectedRoi.alarm_type.relay=false:'';
