...
export class ReportAlertComponent implements OnInit, AfterViewInit {
  ...
  ConfigVarForm: FormGroup = new FormGroup({
    ...
    riroPanelBufferTime: new FormControl(null, [
      Validators.pattern(new RegExp("^(?!-)[0-9]+$")),
    ]),

    riroFrameBufferTime: new FormControl(null, [
      Validators.pattern(new RegExp("^(?!-)[0-9]+$")),
    ]),
  });
  aiSolutions: any[] = [
  ...
  constructor(
    ...
    this.aiSolutionsList=this.configService.aiSolutionsList
  }
  ...
  GetConfigValues() {
    this.ReportService.GetConfigValues().subscribe((response: any) => {
      if (response.success) {
        ...
        this.ConfigVarForm.get("gridSize").setValue(
          response.message?.grid_size
        );
      }
    });
  }
