...
import { ConfigarationComponent } from "../configaration/configaration.component";
interface Response {
  message: any;
  success: boolean;
}
...
export class CameraSettingsComponent implements OnInit, OnDestroy, AfterViewInit
{
  ...
  alarmVersions: any[] = [
    { data: "type1", label: "type-1", key: 0 },
    { data: "type2", label: "type-2", key: 1 },
    { data: "type3", label: "type-3", key: 2 },
  ];
  ...
  cameraAddMethod: FormControl = new FormControl("camera_ip");
  headers: any[] = [
  ...
  constructor(
    ...
    this.aiSolutions=this.configService.aiSolutionsList
    this.IP = server.IP;
  ...
  ngOnInit(): void {
  // this.configarationcomponent.ngOnInit()
  ...
  this.cameraAddMethod.valueChanges.subscribe((value:any)=>{
    this.isSuccess=false
    this.isFail=false
  ...
  SetAISolution(camera: any, event: any, solution: any) {
    ...
    console.log('this is cameradetails from the setaifunction',cameraDetails)
