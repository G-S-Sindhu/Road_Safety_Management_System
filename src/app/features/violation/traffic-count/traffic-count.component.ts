...
export class TrafficCountComponent implements OnInit, OnDestroy {
  selectedMoments: any=null
  ...
  constructor(public service: TrafficCountService) {
  ...
  Dataread() {
    ...
    this.service
      .GetTCLiveData(
        this.selectedCamera ? this.selectedCamera.data : null,
        this.selectedDepartment ? this.selectedDepartment.data : null
      )
      .subscribe(
        (response: any) => {
          this.lastUpdatedDate = this.service.dateTransformbyPattern(
            new Date(),
            "YYYY-MM-dd HH:mm:ss"
          );
  ...
  datesUpdated(dateEvent: any) {
    console.log(this.selectedMoments,'selectedmoments')
    if(this.selectedMoments!=null&&this.selectedMoments.startDate!=null){
    this.isLive = false;
    clearInterval(this.interval);
   
    console.log(dateEvent, "event date");
    this.fromDate = dateEvent.startDate
      ? dateEvent.startDate.format("YYYY-MM-DD HH:mm:ss")
      : null;
    this.toDate = dateEvent.endDate
      ? dateEvent.endDate.format("YYYY-MM-DD HH:mm:ss")
      : null;
  ...
  getCameraList() {
    ...
    this.service
      .GetTCCameraDetails(
        !this.isLive ? this.fromDate : null,
        !this.isLive ? this.toDate : null
      )
      .subscribe((data: any) => {
  ...
  onDepartmentIdSelect(event: any) {
    ...
    console.log(
      event,
      "this is selected department from the traffic count component"
    );
    // console.log(this.selectedDepartment,'this is selected department from the traffic count component')
    if (!this.isLive) {
      this.GetTCDataByFilters();
    } else {
      this.Dataread();
