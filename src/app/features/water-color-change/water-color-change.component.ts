...
var data:any[]=[]
@Component({
  selector: 'app-water-color-change',
  templateUrl: './water-color-change.component.html',
  styleUrls: ['./water-color-change.component.css']
})
export class WaterColorChangeComponent {
  data: any[] = []
  zoom = 100
  selectedPlant:any
  selectedArea:any
...
Hooter(IP:string,delay:number){
  this.http.get("https://192.168.1.22/Delay*100@").subscribe()
  this.http.get(IP+'/Delay*'+delay+'@')
}

Relay(relayIp:any){
   this.http.get(relayIp).subscribe()
